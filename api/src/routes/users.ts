...
usersRouter.get('/', async (req, res) => {
  ...
  const ciParser = !regexParams.number.test(ci as string) || ci === undefined ? undefined : +ci;

  const nameParser = name === '' ? undefined : (name as string);

  const deliveredParser = !regexParams.delivered.test(delivered as string) ? undefined : Boolean(delivered);

  const dateParser = !regexParams.date.test(date as string) ? undefined : (date as string);

  const userCountParser = !regexParams.number.test(userCount as string) || userCount === undefined ? 0 : +userCount;
